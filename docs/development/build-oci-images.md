...
The `build-Image-oci.yaml` workflow allows you to build custom WSL root
...
4. Fill in the required parameters:
    - **Flavor**: Choose from `ubuntu`, `arch`, `alpine`, `debian`, or
      `opensuse`
...
```bash
gh workflow run build-Image-oci.yaml \
  --field flavor=arch \
...
curl -X POST \
  ...
  https://api.github.com/repos/OWNER/REPO/actions/workflows/build-Image-oci.yaml/dispatches \
...
-   **alpine**: Alpine Linux miniImage from
    `https://dl-cdn.alpinelinux.org/alpine/vX.Y/releases/x86_64/alpine-miniImage-X.Y.Z-x86_64.tar.gz`
-   **debian**: Debian Image from
...
-   Uploads the processed Image as an artifact for the next job

### 2. build-and-push-Image

This job runs for all flavors and:

-   Downloads the appropriate base Image for each flavor
...
1. **Pulled and converted back to WSL Image**:
...
-   **Downloads base Image** from official distribution sources
...
-   **Creates optimized WSL Image** with all customizations applied
...
3. **Image Download**: Fetches appropriate base Image for each flavor
