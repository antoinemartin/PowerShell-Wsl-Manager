...
initialize_user_home() {
    ...
    if [ ! -z "$homedir" ]; then
        ...
        su -l $user -c "gpg -k && gpgconf --kill keyboxd || /bin/true" >/dev/null 2>&1
    fi
}
...
configure_alpine() {
    ...
    echo "Adding packages..."
    grep -q edge/community /etc/apk/repositories || echo "https://dl-cdn.alpinelinux.org/alpine/edge/community/" >>/etc/apk/repositories
    grep -q edge/testing /etc/apk/repositories || echo "https://dl-cdn.alpinelinux.org/alpine/edge/testing/" >>/etc/apk/repositories
    apk update --quiet
